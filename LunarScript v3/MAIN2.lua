...
local MultiTeleport = {
    ...
    Markers = {},
    MaxCoordinates = 100,
    TeleportSpeed = 1, -- Default speed for Multi Teleport
    Delay = 1.0 -- Default delay 1 second
}
...
local AFKSystem = {
    ...
    AutoJumpInterval = 5, -- Default 5 minutes
    LastActionTime = tick()
}

-- AI Chat System Configuration (100% ORIGINAL CODE)
local AIChat = {
...
NavigationFrame.CanvasSize = UDim2.new(0,0,0,350) -- Enlarged to accommodate AFK
...
NavContainer.Size = UDim2.new(1,0,0,350) -- Enlarged to accommodate AFK
...
-- =================================================================
-- MULTI TELEPORT FRAME (MODIFIED WITH SPEED AND DELAY BUTTONS)
-- =================================================================
...
MultiTeleportFrame.CanvasSize = UDim2.new(0,0,0,500) -- Enlarged for speed and delay
...
-- ADDITION: Teleport Speed Section
...
-- ADDITION: Delay Settings Section with "<" and ">" buttons
...
-- Coordinate Input Section (moved down)
...
-- =================================================================
-- TOOLS FRAME (Without Clear Tools)
-- =================================================================
...
-- Builder Tools GUI that will appear below visual tools list
...
-- Scrolling frame for part types
...
-- Undo/Redo Buttons for Builder Tools
...
local function CreateDeleteTool()
    -- Create ScreenGui for Delete Tool
    ...
    -- Undo/Redo GUI (Center Top - without background)
    ...
    redoButton.BackgroundColor3 = Color3.fromRGB(100, 100, 100) -- Disabled for redo
    redoButton.TextColor3 = Color3.fromRGB(255, 255, 255)
    redoButton.Text = ">"
    redoButton.Font = Enum.Font.GothamBold
    ...
    -- Highlight function with Box + Fill
    local function createHighlight(part)
        ...
        -- Create Highlight (faint red color over entire object)
        ...
        highlight.Transparency = 0.7 -- Slightly faint
    ...
    deleteButton.MouseButton1Click:Connect(function()
        if selectedPart and selectedPart.Parent then
            -- Clone part WITHOUT highlight/selection box
            local partClone = selectedPart:Clone()
            
            -- Clean all SelectionBox and Highlight from clone
    ...
    undoButton.MouseButton1Click:Connect(function()
        if #deletedParts > 0 then
            ...
            -- Restore part that has been cleaned from highlight
            local restored = lastPart.Part:Clone()
            
            -- Ensure no highlight/selection box in restored part
...
local function ToggleDeleteMode()
    ...
    if deleteModeActive then
        ...
        -- Initialize delete tool if not already created
        if not deleteGUI then
            CreateDeleteTool()
        end
        
        -- Activate delete mode (like holding a tool)
        ...
        -- Deactivate delete mode (like releasing tool)
        if mouseConnection then
            mouseConnection:Disconnect()
            mouseConnection = nil
        end
...
local function ToggleBuilderTools()
    ...
    if builderModeActive then
        ...
        -- Show builder tools GUI below visual tools list
        ...
        -- Activate builder mode (like holding a tool)
        local mouse = localPlayer:GetMouse()
        ...
        -- Deactivate builder mode (like releasing tool)
        if builderMouseConnection then
            builderMouseConnection:Disconnect()
            builderMouseConnection = nil
        end
        
        -- Hide builder tools GUI
        BuilderToolsGUISection.Visible = false
        BuilderUndoRedoFrame.Visible = false
    end
end
...
local function EnableAntiAFK()
    ...
    -- Simulate mouse activity to prevent AFK
    spawn(function()
        local mouse = localPlayer:GetMouse()
...
local function EnableAutoJump()
    ...
    spawn(function()
        while AFKSystem.AutoJumpEnabled do
            wait(interval * 60) -- Convert minutes to seconds
...
-- =================================================================
-- AI CHAT BOT FRAME (100% ORIGINAL CODE WITHOUT MODIFICATION)
-- =================================================================
...
-- UIListLayout to arrange messages vertically
...
-- =================================================================
-- AI CHAT FUNCTIONS (100% ORIGINAL CODE WITHOUT MODIFICATION)
-- =================================================================

-- Function to add message with vertical layout
local function AddMessageToChatVertical(sender, messageText)
    ...
    if sender == "user" then
        backgroundColor = Color3.fromRGB(0, 120, 215)  -- Blue for user
        textColor = Color3.fromRGB(255, 255, 255)
    else
        backgroundColor = Color3.fromRGB(60, 60, 80)   -- Gray for AI
        textColor = Color3.fromRGB(255, 255, 255)
    end
...
-- HTTP Request function for AI API (100% ORIGINAL CODE)
...
-- Test connection function (100% ORIGINAL CODE)
...
-- Send message function (100% ORIGINAL CODE)
...
-- =================================================================
-- MULTI TELEPORT FUNCTIONS (MODIFIED WITH SPEED AND DELAY)
-- =================================================================
...
local function UpdateCoordinatesList()
    ...
    for i, coord in ipairs(MultiTeleport.Coordinates) do
        ...
        -- DELETE BUTTON ONLY APPEARS IF THERE'S AN ITEM
...
local function TeleportToCoordinates()
    ...
    for i = #MultiTeleport.Coordinates, 1, -1 do
        ...
        wait(delay) -- Using adjustable delay
    end
...
-- =================================================================
-- CONNECT EVENT HANDLERS (ADDED AFK AND DELAY BUTTONS)
-- =================================================================
...
AFKButton.MouseButton1Click:Connect(ShowAFK) -- ADDITION

-- AI Chat handlers (100% ORIGINAL CODE)
...
-- DELAY BUTTONS HANDLERS (NEW ADDITION)
...
-- Validate Multi Teleport speed input
...
-- Validate Multi Teleport delay input
...
-- AFK Anti-Kick handlers (ADDITION)
...
-- Validate Auto Jump interval input
...
-- Initialize Builder GUI
InitializeBuilderGUI()

-- Add demo messages to AI chat (WITHOUT MODIFICATION)
spawn(function()
    wait(3)
